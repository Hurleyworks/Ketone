...
*/
    ...
    */
        ...
        virtual unsigned short readUnsignedShortBigEndian()
        {
            char temp[2];

            if (read (temp, 2) == 2)
                return (unsigned short)ByteOrder::bigEndianShort (temp);

            return 0;
        }

        virtual uint32_t readUnsignedIntBigEndian()
        {
            char temp[4];

            if (read (temp, 4) == 4)
                return (uint32_t)ByteOrder::bigEndianInt (temp);
