...
PixelBufferHandle OpenglWindow::renderImage (PixelBuffer&& pixelBuffer, bool returnRender)
{
    ImageInfo spec;
    spec.width = pixelBuffer.spec.width;
    spec.height = pixelBuffer.spec.height;
    spec.channels = 4;
    spec.dataType = ImageDataType::UINT8_DATA;

    // no rendering while the pixel buffer is being resized
    // on the render thread
    if (pixelBuffer.spec.dirty)
    {
        if (textureID)
        {
            glDeleteTextures (1, &textureID);
        ...
        return returnRender ? getRender (spec) : nullptr;
    }

    if (pixelBuffer.spec.dataType == ImageDataType::FLOAT_DATA)
    {
        renderFloatImage (std::move (pixelBuffer));
...
void OpenglWindow::renderFloatImage (PixelBuffer&& pixelBuffer)
{
    if (!textureID)
    {
        LOG (DBUG) << pixelBuffer.spec.width << ", " << pixelBuffer.spec.height;

        glCreateTextures (GL_TEXTURE_2D, 1, &textureID);

        if (pixelBuffer.spec.channels == 3)
        {
            glTextureStorage2D (textureID, 1, GL_RGBA32F, pixelBuffer.spec.width, pixelBuffer.spec.height);
            glTextureSubImage2D (textureID, 0, 0, 0, pixelBuffer.spec.width, pixelBuffer.spec.height, GL_RGB, GL_FLOAT, pixelBuffer.floatPixels.data());
        }
        else if (pixelBuffer.spec.channels == 4)
        {
            //  glTexImage2D (textureID, 1, GL_RGBA8, pixelBuffer.spec.width, pixelBuffer.spec.height, 0, GL_RGBA, GL_FLOAT, pixelBuffer.floatPixels.data());
            glTextureStorage2D (textureID, 1, GL_RGBA32F, pixelBuffer.spec.width, pixelBuffer.spec.height);
            glTextureSubImage2D (textureID, 0, 0, 0, pixelBuffer.spec.width, pixelBuffer.spec.height, GL_RGBA, GL_FLOAT, pixelBuffer.floatPixels.data());
        }
...
PixelBufferHandle OpenglWindow::getRender (const ImageInfo spec)
{
    PixelBufferHandle render = PixelBuffer::createHandle();
    render->init (spec);
    glReadPixels (0, 0, spec.width, spec.height, GL_RGBA, GL_UNSIGNED_BYTE, render->uint8Pixels.data());
                           ...
                           int nSamples, unsigned int glMajor, unsigned int glMinor)
{
    ...
    // input events
    glfwSetCharCallback (window, [] (GLFWwindow* window, unsigned int codepoint) { get (window).getInput().consumeCharacter (codepoint); });
    glfwSetKeyCallback (window, [] (GLFWwindow* window, int key, int scanCode, int action, int mods) { get (window).getInput().consumeKey (key, scanCode, action, mods); });
